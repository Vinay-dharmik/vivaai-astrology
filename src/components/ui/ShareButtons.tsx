...
export function ShareButtons({ title, text, url }: ShareButtonsProps) {
  ...
  const shareUrl = url || (typeof window !== "undefined" ? window.location.href : "https://vivaai.in");
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      >
        {copied ? <Check className="w-4 h-4 text-mystic-green" /> : <Copy className="w-4 h-4 text-[var(--text-muted)]" />}
      </button>
      ...
      {typeof navigator !== "undefined" && "share" in navigator && (
        <button
          ...
          className="w-8 h-8 rounded-lg bg-gold-400/10 hover:bg-gold-400/20 flex items-center justify-center transition-colors md:hidden"
          aria-label="Share"
        >
